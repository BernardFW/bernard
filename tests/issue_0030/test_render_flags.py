...
def test_multi_sentence():
    with patch_conf({"I18N_TRANSLATION_LOADERS": []}):
        ...
        assert wd.get("HELLO", flags={"gender": "male"}) == ["hello boy", "wassup?"]

        assert wd.get("HELLO", flags={"gender": "female"}) == ["hello girl", "wassup?"]
...
def test_multi_sentence_reverse():
    with patch_conf({"I18N_TRANSLATION_LOADERS": []}):
        ...
        assert wd.get("HELLO", flags={"gender": "male"}) == ["hello", "wassup boy?"]

        assert wd.get("HELLO", flags={"gender": "female"}) == ["hello", "wassup girl?"]
